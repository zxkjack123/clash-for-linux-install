...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --report)
      ...
      ;;
    --trial)
      [[ $# -lt 2 ]] && { echo "ERROR: --trial requires a value" >&2; exit 2; }
      TRIAL_SECONDS="$2"
      DRY_RUN=false
      REPORT=true
      shift 2
      ;;
    --require-url)
      [[ $# -lt 2 ]] && { echo "ERROR: --require-url requires a value" >&2; exit 2; }
      REQUIRE_URLS+=("$2")
      shift 2
      ;;
    --require-timeout)
      [[ $# -lt 2 ]] && { echo "ERROR: --require-timeout requires a value" >&2; exit 2; }
      REQUIRE_TIMEOUT="$2"
      shift 2
      ;;
    --print-unset)
...
should_unset_proxy_auto() {
  ...
  if [ -f "$CLASH_PROXY_STATE_FILE_EFFECTIVE" ] || [ -f "$CLASH_PROXY_STATE_FILE_LEGACY" ]; then
    return 0
  fi
  # 2) APT temp proxy file generated by _set_system_proxy
  if [ -f "$CLASH_APT_PROXY_STAGE_FILE_EFFECTIVE" ] || [ -f "$CLASH_APT_PROXY_STAGE_FILE_LEGACY" ]; then
    return 0
  fi
...
if $do_unset_proxy; then
  ...
  if $DRY_RUN; then
    say "[dry-run] rm -f $CLASH_APT_PROXY_STAGE_FILE_EFFECTIVE $CLASH_APT_PROXY_STAGE_FILE_LEGACY"
  else
    rm -f "$CLASH_APT_PROXY_STAGE_FILE_EFFECTIVE" "$CLASH_APT_PROXY_STAGE_FILE_LEGACY" 2>/dev/null || true
  fi
